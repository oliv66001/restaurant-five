...

{
    ...
    protected function voteOnAttribute($attribute, $dishes, TokenInterface $token): bool
    {
...
use App\Security\Voter\DishesVoter;
 ...
 * Summary of DishesVoter
 */
class DishesVoter implements VoterInterface

{
    ...
    protected function voteOnAttribute($attribute, $dishes, TokenInterface $token): bool
    {
        ...
        // On vérifie si l'utilisateur est admin
        if($this->decisionManager->decide($token, ['ROLE_ADMIN'])) return true;

        // On vérifie les permissions
        switch($attribute){
            case self::EDIT:
                // On vérifie si l'utilisateur peut éditer
                return $this->decisionManager->decide($token, ['ROLE_dishes_ADMIN']);
                break;
            case self::DELETE:
                // On vérifie si l'utilisateur peut supprimer
